...
| `quote_matcher.py` | Quote-fidelity primitives. Public `verify_quote(quote, opinion_text, *, was_ocrd=False) -> QuoteVerification` (+ `QuoteMatch` enum), exported from the package. **Buckets structurally, not on a ratio (2026-09-22):** locate with the oversized chunk, re-align the quote to the span it actually matches -- the old fixed 1.5w chunk capped `SequenceMatcher.ratio()` at 2w/2.5w = 0.80, so VERBATIM was reachable only by exact substring and `if best > 0.95: break` was dead code -- then diff at the WORD level. Junk only (star pagination, footnote markers, punctuation, quote marks, hyphenation, possessive apostrophes, ellipsis elisions, bracketed alterations on EITHER side, span overhang) -> `VERBATIM`; any word substituted/added/dropped -> `CLOSE`, each named in the new `QuoteVerification.alterations` tuple (persisted per quote in the `quote_check` column); alignment < 0.6 -> `FABRICATED`. `similarity` is word-content similarity with licensed material removed, so VERBATIM is exactly 1.0 -- **do not reintroduce a similarity threshold for bucketing**, a `shall`->`may` swap scores ~0.96 on any char scale. Houses `_normalize_quote_text`, `_best_match_with_passage`, `_extract_passage` (re-exported from `proposition_pipeline` for compat) and `_normalize_ocr_confusions` (conservative `rn`->`m` / `O`->`0` / `l`->`1`, applied only when `was_ocrd`). Design + the 63-quote calibration diff: `docs/plans/2026-09-22-quote-matcher-structural-buckets.md`. |
...
| `test_assessment_regression.py` | Offline assessment baselines. **Recalibrated 2026-09-22** for the quote matcher's structural buckets. v1: Withers 14/19 yellows, greens 8 exact / 3 over-flags; A/B 56/61 (payne 24 + wainwright 32); lenient set {payne-03}. **assess-v2**: Withers 16/19 yellows / reds 3/3 / greens 5 over-flags (≤2 guardrail miss, adjudicated + the withers-21 band removal), A/B 55/61 (90%), lenient set {payne-03} only |
