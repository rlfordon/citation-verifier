...
| `quote_matcher.py` | Quote-fidelity primitives. Public `verify_quote(quote, opinion_text, *, was_ocrd=False) -> QuoteVerification` (+ `QuoteMatch` enum), exported from the package. **Buckets structurally, not on a ratio (2026-09-22):** locate with the oversized chunk, re-align the quote to the span it actually matches -- the old fixed 1.5w chunk capped `SequenceMatcher.ratio()` at 2w/2.5w = 0.80, so VERBATIM was reachable only by exact substring and `if best > 0.95: break` was dead code -- then diff at the WORD level. Junk only (star pagination, footnote markers, punctuation, quote marks, hyphenation, possessive apostrophes, ellipsis elisions, bracketed alterations on EITHER side, span overhang) -> `VERBATIM`; any word substituted/added/dropped -> `CLOSE`, each named in the new `QuoteVerification.alterations` tuple, with `altered_words` counting the words they touch (both persisted per quote in the `quote_check` column; `_quote_floor` keys its lone-word exemption on the count); alignment < 0.6 -> `FABRICATED`. `similarity` is word-content similarity with licensed material removed, so VERBATIM is exactly 1.0 -- **do not reintroduce a similarity threshold for bucketing**, a `shall`->`may` swap scores ~0.96 on any char scale. Houses `_normalize_quote_text`, `_best_match_with_passage`, `_extract_passage` (re-exported from `proposition_pipeline` for compat) and `_normalize_ocr_confusions` (conservative `rn`->`m` / `O`->`0` / `l`->`1`, applied only when `was_ocrd`). Design + the 63-quote calibration diff: `docs/plans/2026-09-22-quote-matcher-structural-buckets.md`. |
...
| `jev_shadow.py` | **Shadow logging for TypeSafe's Jev model** (2026-09-19). Jev is a fast, cheap classifier, not a text generator: you send text plus typed questions and get probabilities back (~0.5 s and ~$0.0005 per claim). `run_jev_shadow(workdir)` logs Jev's answers for every claim with opinion text to `jobs/jev_shadow.jsonl` and **changes nothing else** -- never claims.csv, verdicts, or the report; per-claim failures are recorded and a whole-step failure only prints a warning. Per claim: one locator request (ranks <=250 ID-tagged passages; callable on its own as `locate()` + `keep_indices()` + `join_passages()`) then the rubric on the top-5 passages under two state framings. Logs raw answers plus `gate_v0` (original `min(relation, states_it, same_issue)`) and `gate_v1` (mean of the `as_written` question under both framings -- the phrasing-loop champion) and a deterministic `eligible` flag (clean VERIFIED, no quote problem, no crosscheck flag). **Opt-in, because it sends propositions and opinion excerpts to a third-party API:** the `jev-shadow` verb, or inside `full` when `JEV_SHADOW=1` (env/.env; set it to 0 for non-public documents). Needs `TYPESAFE_API_KEY` and `pip install -e ".[jev]"`. Idempotent (resume key = claim_id + `RUBRIC_VERSION`); **bump `RUBRIC_VERSION` on any change to the questions, state layout, or excerpting** -- rows under different versions are not comparable. Purpose: every run's LLM verdicts become labels for Jev, so the auto-Green gate can be re-checked on fresh briefs (`tools/jev_shadow_report.py`). Nothing acts on Jev's answers yet. Background: `scratch/jev/RESULTS.md`, `scratch/jev/LOOP.md`, `scratch/jev/LOCATOR_ASSESS.md` (feeding the locator's passages to the LLM assessment: costed and parked, not run), `scratch/jev_citation_checking_research.md` |
...
| `test_assessment_regression.py` | Offline assessment baselines. **Recalibrated 2026-09-22** for the quote matcher's structural buckets. v1: Withers 14/19 yellows, greens 9 exact / 2 over-flags; A/B **57/61** (payne 24 + wainwright 33 — up from 56/61 because the A/B corpora's quote columns were stale, predating `quote_floor`); lenient set {payne-03} (was {payne-03, payne-58}). **assess-v2**: Withers 16/19 yellows / reds 3/3 / greens 4 over-flags (≤2 guardrail miss, adjudicated 2026-06-12), A/B 55/61 (90%), lenient set {payne-03} only |
| `build_assessment_corpora.py` | Builds/refreshes `tests/data/assessment_corpora/` (idempotent). Re-runs `check_quotes` on **every** corpus so the quote columns track the current matcher, and rewrites only its own `PROMPT_VERSION` rows in each cassette — it used to truncate the file, destroying the assess-v2 live recordings on any rebuild |
| `test_ab_runner.py` | Offline tests for `tools/ab_test_runner.py` (RecordedExecutor seam; payne 24/27, wainwright 33/34) |
...
| `tools/jev_shadow_report.py` | Joins each run's `jobs/jev_shadow.jsonl` to its claims.csv `support` column: claims the Jev gate would clear vs bad clears, by threshold and by run (`--gate gate_v0\|gate_v1`), with the rule-of-three bound on the true bad-clear rate |
